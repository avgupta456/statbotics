...
class TeamFilterSet(django_filters.FilterSet):
    ...
    years_active = django_filters.NumberFilter(method='get_years_active',
    ...
    def get_years_active(self, queryset, field_name, value):
        if value:
            return queryset.filter(years_active=value)
    ...
    class Meta:
        model = Team
        fields = ['team', 'active', 'years_active', 'country', 'state',
                  'district']
