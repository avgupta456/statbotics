...
def process_year(
    ...
    component_team_events_dict: Dict[str, List[Tuple[float, float, float, bool]]] = {}
    component_team_matches_dict: Dict[int, List[Tuple[float, float, float]]] = {}
    component_team_match_ids: Dict[str, Tuple[float, float, float]] = {}
    ...
    for match in sorted_matches:
        ...
        for teams, epa_dict in [(red, red_epa_pre), (blue, blue_epa_pre)]:
            for t in teams:
                ...
                if team_event_key not in team_events_dict:
                    ...
                    if USE_COMPONENTS:
                        auto_epa, teleop_epa, endgame_epa = component_team_match_ids[
                            get_team_match_key(t, match.key)
                        ]
                        component_team_events_dict[team_event_key] = [
                            (auto_epa, teleop_epa, endgame_epa, match.playoff)
                        ]
        ...
        if USE_COMPONENTS:
            ...
            red_endgame_err = (match.red_endgame or 0) - (match.red_endgame_epa_sum or 0)  # type: ignore
            blue_endgame_err = (match.blue_endgame or 0) - (match.blue_endgame_epa_sum or 0)  # type: ignore
            for teams, auto_epa_pre, endgame_epa_pre, auto_err, endgame_err in [
                (red, red_auto_epa_pre, red_endgame_epa_pre, red_auto_err, red_endgame_err),  # type: ignore
                (blue, blue_auto_epa_pre, blue_endgame_epa_pre, blue_auto_err, blue_endgame_err),  # type: ignore
            ]:
                for t in teams:
                    ...
                    t_epa = max(0, team_epas[t] - a_epa - e_epa)
                    team_auto_epas[t] = a_epa
                    team_teleop_epas[t] = t_epa
                    team_endgame_epas[t] = e_epa
                    team_event_key = get_team_event_key(t, event_key)
                    component_team_events_dict[team_event_key].append(
                        (a_epa, t_epa, e_epa, match.playoff)
                    )
                    component_team_matches_dict[t].append((a_epa, t_epa, e_epa))
    ...
    for team_match in completed_team_matches:
        ...
        if USE_COMPONENTS:
            auto, teleop, endgame = component_team_match_ids[match_key]
    ...
    for team_event in sorted(team_events, key=lambda e: (e.week, e.time)):
        ...
        # TODO: revisit if we need epa_max, epa_diff
        ...
        if USE_COMPONENTS:
            component_epas = [
                obj[0:2] for obj in component_team_events_dict.get(key, upcoming_epas)
            ]
            qual_component_epas = [
                obj[0:2] for obj in component_team_events_dict.get(key, upcoming_epas) if not obj[1]  # type: ignore
            ]

            auto_epas = [obj[0] for obj in component_epas]
            teleop_epas = [obj[1] for obj in component_epas]
            endgame_epas = [obj[0] for obj in component_epas]

            auto_qual_epas = [obj[0] for obj in qual_component_epas]
            teleop_qual_epas = [obj[1] for obj in qual_component_epas]
            endgame_qual_epas = [obj[0] for obj in qual_component_epas]
