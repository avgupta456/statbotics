...
export default function ModelsBlogPage() {
  return (
    <PageLayout
    ...
    >
      ...
      <ul>
        <li>
          <strong>Wins Baseline</strong>: This simple model only considers a team&apos;s record. The
          alliance with the most wins that year is predicted to win. This model is roughly how a
          human would intuitively predict the outcome of a match, and is a good baseline for
          evaluating more complex models.
        </li>
        <li>
          <strong>Offensive Power Rating (OPR)</strong>: OPR uses linear algebra to minimize the sum
          of squared errors between the predicted and actual scores. We evaluate a variant of OPR
          called ixOPR that incorporates a prior during ongoing events. TBA Insights and Statbotics
          both incorporate ixOPR.
        </li>
        <li>
          ...
          on the difference between the predicted and observed winning margin. Caleb Sykes modified
          Elo for FRC, and we include both his original mode and the Statbotics implementation.
        </li>
        <li>
      ...
      <ul>
        <li>
          <strong>Predictive Power</strong>: How well does the model predict the outcome of a match?
          We evaluate this by comparing the model&apos;s predictions to the actual outcomes.
        </li>
        <li>
          <strong>Interpretability</strong>: How easy is it to understand the model&apos;s
          predictions and incorporate them into a strategy? Ratings in point units and ratings that
          can be separated into components are more interpretable.
        </li>
        <li>
          <strong>Accessibility</strong>: How easy is it to use the model? Models that are available
          on a website or API are more accessible than models that require user calculations.
        </li>
      </ul>
      ...
      <p>
        ...
        indicates no skill. Going back to 2002, we evaluate models on 160,000 matches, with special
        emphasis on 15,000 champs matches and the 80,000 recent matches since 2016.
      </p>
      <p>
        ...
        This reflects the extent to which the model is able to predict match outcomes beyond the eye
        test.
      </p>
      ...
      <p>
        ...
        comparison, the OPR model has a 68% accuracy, the Elo model has a 69% accuracy, and the EPA
        model has a 70% accuracy. Since 2016, these numbers are 66% (Wins), 70% (OPR), 71% (Elo),
        and 72% (EPA). While the EPA model outperforms the OPR/Elo models by only 1-2%, in relation
        to the wins baseline,{" "}
        <strong>the EPA model outperforms the baseline by ~20% more than the OPR/Elo models</strong>
      ...
      <p>
        ...
        EPA to Elo and OPR individually, Statbotics previously used a combination of both, which has
        improved accuracy compared to either model alone. Still, the EPA model individually
        outperforms this ensemble, and future EPA iterations can ensemble with other models to reach
        even higher performance. Second, while the EPA model significantly outperforms other models
        during the season, this does not translate to champs. EPA stabilizes to an accurate
        prediction faster during the season, but by champs, Elo has caught up and is roughly
        equivalent to EPA.
      </p>
      ...
      <p>
        ...
        ratings across years as well (blog post coming soon). In summary, the EPA model combines the
        best of both worlds: point units, separable into components, and comparable across years.
      </p>
      <h3>Accessibility</h3>
      <p>
      ...
      <h3>Conclusion</h3>
      <p>
        The EPA model is the best performing model in 15 of the 20 years, including six of the last
        seven. The EPA model is the most interpretable model, with point units, separable
        components, and year-normalized ratings. Finally, the EPA model is highly accessible,
        available on the Statbotics website and through Python API. In summary, we highly recommend
        the EPA model for teams and scouting systems. Please reach out to us if you have any
