...
def get_data(start_year, end_year):
    ...
    for year in range(start_year, end_year+1):
        ...
        for m in utils.loadProcessedMatches(year):
            ...
            for i in range(len(m.red)):
                ...
                if(append):
                    team_matches.append([id, year, event, match, time,
                                         m.red[i], start_elo, end_elo,
                                        elo_diff])
                    id += 1
            ...
            for i in range(len(m.blue)):
                ...
                if(append):
                    team_matches.append([id, year, event, match, time,
                                         m.blue[i], start_elo, end_elo,
                                         elo_diff])
                    id += 1

    team_matches = pd.DataFrame(team_matches,
                                columns=["id", "year", "event", "match",
                                         "time", "team", "elo_start",
                                         "elo_end", "elo_diff"])
    ...
    team_events = pd.DataFrame(team_events,
                               columns=["id", "year", "event", "time", "team",
    ...
    champ_keys = ['arc', 'cars', 'carv', 'cur', 'dal', 'dar', 'gal', 'hop',
                  'new', 'roe', 'tes', 'tur']
    ...
    for year in range(start_year, end_year+1):
        ...
        for team in year_data_events['team'].unique():
            ...
            team_data_matches = year_data_matches[
                year_data_matches.team == team]
            ...
            if len(set(champ_keys).intersection(
               set(team_data_events["event"]))) == 0:
                elo_pre_champs = elo_end
            else:
                elo_pre_champs = round(team_data_events[
                    team_data_events.event.isin(champ_keys)]
                    ["elo_start"].iloc[0])
            ...
            team_years.append([id, year, team, name, country, state, district,
                               elo_start, elo_pre_champs, elo_end, elo_mean,
                               elo_max, elo_diff])
            id += 1

    team_years = pd.DataFrame(team_years,
                              columns=["id", "year", "team", "name", "country",
    ...
    teams = pd.DataFrame(teams,
                         columns=["team", "name", "country", "state",
                                  "district", "years_active", "active", "elo",
                                  "elo_recent", "elo_mean", "elo_max",
                                  "elo_max_year"])
    ...
    for year in range(start_year, end_year+1):
        ...
        for event in year_data["event"].unique():
            ...
            events.append([id, year, event, elo_max, elo_top8, elo_top24,
                           elo_mean, elo_sd])
            id += 1

    events = pd.DataFrame(events,
                          columns=["id", "year", "event", "elo_max",
    ...
    for year in range(start_year, end_year+1):
        ...
        years.append([year, elo_max, elo_1p, elo_5p, elo_10p, elo_25p,
                      elo_median, elo_mean, elo_sd, acc, mse])

    years = pd.DataFrame(years,
                         columns=["year", "elo_max", "elo_1p", "elo_5p",
                                  "elo_10p", "elo_25p", "elo_median",
...
def push_data(data):
    ...
    engine = create_engine('mysql+pymysql://' + constants.CLOUDSQL_USER +
                           ...
                           '@' + constants.CLOUDSQL_HOST +
                           ':' + constants.CLOUDSQL_PORT +
                           '/' + constants.CLOUDSQL_DATABASE)

    team_matches.to_sql('rankings_teammatch',
                        engine,
                        if_exists='replace',
                        index=False)

    team_events.to_sql('rankings_teamevent',
                       engine,
                       if_exists='replace',
                       index=False)

    team_years.to_sql('rankings_teamyear',
                      engine,
                      if_exists='replace',
                      index=False)
