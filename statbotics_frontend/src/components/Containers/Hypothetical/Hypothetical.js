...
export default function Hypothetical() {
  ...
  function updateRed1(e) {
    setRed1(e.target.value);
  }
  function updateRed2(e) {
    setRed2(e.target.value);
  }
  function updateRed3(e) {
    setRed3(e.target.value);
  }

  function updateBlue1(e) {
    setBlue1(e.target.value);
  }
  function updateBlue2(e) {
    setBlue2(e.target.value);
  }
  function updateBlue3(e) {
    setBlue3(e.target.value);
  }
    ...
    <Card className={styles.main}>
      ...
      <div className={classes.container}>
        <Typography className={classes.type}>
          {button === 0
            ? "Enter teams and click 'Predict Match'"
            : winProb > 50
            ? `Elo predicts Red Alliance has a ${winProb}% chance of winning.`
            : `Elo predicts Blue Alliance has a ${
                100 - winProb
              }% chance of winning.`}
        </Typography>
      </div>
      ...
      <p className={styles.center}>
        Note: If a team does not exist, they are assigned an Elo of 0, so double
        check your team numbers!
      </p>
    </Card>
  );
}
